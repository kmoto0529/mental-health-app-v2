...
> **⚠️ 更新 2026-05-29（社長指示）**: 当初は「リスク検知時は分析より安全案内を優先（分析画面を差し替えてブロック）」としていたが、**リスク検知時も思考のクセ分析は通常通り出力する**よう変更。分析をブロックしない。
> `daRisk()` の算出と `result.safety` フィールドは出力スキーマ維持のため残すが、UI差し替え・AI抑止には使わない。クライシス対応は mood_chat 等の `detectCrisis`（severity≥4 で専門窓口モーダル）が担う。
