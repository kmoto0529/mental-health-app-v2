...
### N. 2026-08-03 週次確認
- **リデザイン並行開発開始**（v0.9.95-beta.3 維持、index.html 変更なし）：`aside-prototype/redesign.html` を新規追加。すぷらん UIリデザイン試作として、オンボーディング9画面（Welcome/主訴7チップ/出来事/考え/気分2スライダー＋行動/なりたい7チップ/仮の心の地図6枠＋見立て/小さな実験＋いつやる/完成）＋ホーム2状態（空/アクティブ）＋ボトムタブ5つを実装。現行βとは並行開発（設計書§16.5の方針）。suplan アセット画像10枚も追加。AIプロンプト14個に変化なし。
...
### U. 2026-09-21 週次確認（v0.9.96-beta.2 維持）
- **demo-v2.html 新規追加**（`cb88ef5`）：心理士×AI 週次ループのデモ版を新規追加。aside-prototype/ 直下に配置。index.html 変更なし。
- **redesign.html：Day0 初回体験 v0.1 実装**（`65d9752`・`6e9d596`）：Day0 体験フロー（D0-01〜D0-15）をデモに実装し、その後 A1-A3 / B1-B2 / C1-C4 / D1-D2 のステップ構成に作り替え。
- **redesign.html：デザイン言語統一＋週1回のふり返りカード追加**（`83934fc`）：redesign.html のビジュアルを統一し「週1回のふり返り」カードを追加。
- 上記はいずれも aside-prototype/redesign.html・demo-v2.html の改修のみ。index.html（本番PWA本体）に変更なし。AIプロンプト14個に変化なし。
