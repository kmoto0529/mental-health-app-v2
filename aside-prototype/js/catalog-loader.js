 ...
 *
 * 役割:
 *   Supabase の public.actions テーブルから
 *   ACTIONS（いっぽの行動カタログ）を取得して上書きする。
 *
 * 設計方針:
 ...
 *   MoyaCatalog.init({
 *     target: ACTIONS,
 *     supabaseUrl: ASIDE_CONFIG.supabaseUrl,
 *     supabaseAnonKey: ASIDE_CONFIG.supabaseAnonKey,
 *     onUpdate: () => render()
 *   });
 *
 * 取得スキーマ（public.actions・2026-05-13〜）:
 *   id / cat / icon / title / description / time_label / technique / domains / is_active
 *   - anon は is_active = true の行のみ SELECT 可能（RLS）
 *   - domains は パイプ区切り（例: 'work|self|sleep'）
 *   - icon は 絵文字1〜2文字（任意）
 *   - 履歴: v3 (2026-05-13〜) で Google Sheets CSV → Supabase REST に切替
 */

(function (global) {
  'use strict';

  const CACHE_KEY = 'moyanomori_actions_cache_v3';   // v3: Supabase actions テーブル由来
  ...
  function normalizeAction(o, lineNo) {
    ...
    if (!o.description) errors.push(`L${lineNo} (${o.id}): description 必須`);
    if (!o.time_label) errors.push(`L${lineNo} (${o.id}): time_label 必須`);
    ...
    return {
      ...
      action: {
        ...
        icon: o.icon || '',
        title: o.title,
        desc: o.description,                 // アプリ内表記は従来通り desc
        time: o.time_label,                  // アプリ内表記は従来通り time
  ...
  async function fetchAndApply(supabaseUrl, supabaseAnonKey, target, onUpdate) {
    if (!supabaseUrl || !supabaseAnonKey) return { skipped: true };
    const endpoint = supabaseUrl.replace(/\/+$/, '') +
      '/rest/v1/actions' +
      '?select=id,cat,icon,title,description,time_label,technique,domains,is_active' +
      '&is_active=eq.true' +
      '&order=id.asc';
    try {
      ...
      if (!Array.isArray(data)) {
        console.warn('[MoyaCatalog] unexpected response shape (expected array)');
        return { failed: true, reason: 'unexpected_shape' };
      }
      const { errors, warnings, validList } = validate(data);
      ...
      saveCache(validList);
      console.log('[MoyaCatalog] supabase から ' + validList.length + ' 件読み込み完了 (skip: ' + errors.length + ', warn: ' + warnings.length + ')');
  ...
  global.MoyaCatalog = {
    ...
    init: function (opts) {
      ...
      if (!supabaseUrl || !supabaseAnonKey) {
        console.log('[MoyaCatalog] supabase 設定未指定 — バンドル既定値で動作');
        return;
      }
      ...
      // 2. 裏で最新取得を試行
      fetchAndApply(supabaseUrl, supabaseAnonKey, target, opts.onUpdate);
    },

    // 手動リロード（管理画面などから）
    refresh: function (supabaseUrl, supabaseAnonKey, target, onUpdate) {
      return fetchAndApply(supabaseUrl, supabaseAnonKey, target, onUpdate);
    },
